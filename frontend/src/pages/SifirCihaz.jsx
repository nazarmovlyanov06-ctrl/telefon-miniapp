...
import {
  ...
  HardDrive, Wrench, Trash2, Banknote, Phone, Radio, History,
...
// Bkz. IkinciEl.jsx'teki aynı isimli sabit/fonksiyon — Sıfır Cihaz'da da
// yeni bir telefon aylarca satılmadan beklerse (hatta 2.El'den bile daha
// hızlı değer kaybeder) bunu işaretleyebilelim diye.
...
// IMEI geçmişi eskiden sadece sifir_cihazlar içinde aranıyordu — aynı
// IMEI'nin 2.El veya Tamir'de de kaydı varsa hiç görünmüyordu.
function DigerEslesmeler({ diger }) {
  if (!diger || (diger.ikinci_el.length === 0 && diger.tamir.length === 0)) return null;
  return (
    <div style={{ background: "rgba(246,162,74,0.1)", border: "1px solid rgba(246,162,74,0.3)", borderRadius: 10, padding: "10px 12px", marginBottom: 14 }}>
      ...
      {diger.ikinci_el.map(s => (
        <div key={"ikincel" + s.id} style={{ fontSize: 12, color: "var(--text)" }}>
          2.El: {s.model} · {s.durum === "satildi" ? "Satıldı" : "Stokta"} · {(s.created_at || "").slice(0, 10)}
        </div>
      ))}
      {diger.tamir.map((t, i) => (
...
export default function SifirCihaz({ user }) {
  ...
  // Kara liste uyarısı — bkz. IkinciEl.jsx'teki aynı mantık.
  ...
  const [imeiModalDiger, setImeiModalDiger] = useState({ ikinci_el: [], tamir: [] });
  ...
  function selectCihaz(c) {
    ...
    setDuzenleForm({
      ...
      kimden: c.kimden || "", kimden_telefon: c.kimden_telefon || "", alis_fiyati: c.alis_fiyati ?? "",
      liste_fiyati: c.liste_fiyati ?? "", fatura_turu: c.fatura_turu || "", notlar: c.notlar || "",
    });
    loadFotolar(c.id);
  }
  ...
  async function loadFotolar(id) {
    setFotoLoading(true);
    try { setFotolar(await api.sifirFotolar(id)); } catch { setFotolar([]); }
  ...
  async function addFoto(e) {
    ...
    reader.onload = async (ev) => {
      try {
        await api.addSifirFoto(selected.id, { foto: ev.target.result, aciklama: "" });
        setFotolar(await api.sifirFotolar(selected.id));
      } catch (_) {}
    };
  ...
  async function deleteFoto(fotoId) {
    try {
      await api.deleteSifirFoto(selected.id, fotoId);
      setFotolar(await api.sifirFotolar(selected.id));
    } catch (_) {}
  }
  ...
  async function submitDuzenle(e) {
    ...
    try {
      await api.updateSifir(selected.id, { ...duzenleForm, alis_fiyati: parseFloat(duzenleForm.alis_fiyati), liste_fiyati: duzenleForm.liste_fiyati ? parseFloat(duzenleForm.liste_fiyati) : null });
  ...
  async function submitMasraf(e) {
    e.preventDefault(); setErr("");
    try {
      await api.sifirMasraf(selected.id, { ...masrafForm, tutar: parseFloat(masrafForm.tutar) });
      setShowMasraf(false);
  ...
  async function openImeiModal(imei, model) {
    ...
    setImeiModalData([]);
    setImeiModalDiger({ ikinci_el: [], tamir: [] });
    try {
      const data = await api.sifirIMEITam(imei);
      setImeiModalData(data.sifir_cihaz || []);
      setImeiModalDiger({ ikinci_el: data.ikinci_el || [], tamir: data.tamir || [] });
  ...
  async function searchIMEI() {
    ...
    setImeiModalDiger({ ikinci_el: [], tamir: [] });
    try {
      const data = await api.sifirIMEI(imeiSon4);
      setImeiGecmis(data.sifir_cihaz || []);
      setImeiModalDiger({ ikinci_el: data.ikinci_el || [], tamir: data.tamir || [] });
    } catch { setImeiGecmis([]); }
  ...
  return (
    <div className="page">
                ...
                return (
                  <div key={c.id} className="card" style={{ padding: 0, overflow: "hidden", cursor: "pointer", margin: 0 }}
                    ...
                    <div style={{ position: "relative", aspectRatio: "1 / 1" }}>
                      <UrunGorsel url={c.gorsel_url} yukle={f => api.sifirCihazGorselYukle(c.id, f)} boyut="100%" />
                      {durgun && (
                        <span style={{
      ...
      {/* IMEI Geçmiş Modalı — bkz. IkinciEl.jsx'teki aynı z-index gerekçesi. */}
      {imeiModal && (
        <div style={{
      ...
      {/* Cihaz Detay Modalı — bkz. IkinciEl.jsx'teki aynı desen. */}
      {selected && (() => {
        const c = selected;
            ...
            <div className="card" style={{ width: "100%", maxWidth: 480, maxHeight: "88vh", overflowY: "auto" }}
              ...
              <div className="card-row" style={{ marginBottom: 10, alignItems: "flex-start" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <UrunGorsel url={c.gorsel_url} yukle={f => api.sifirCihazGorselYukle(c.id, f)} boyut={46} />
                  <div>
              ...
              {showDuzenle ? (
                <form onSubmit={submitDuzenle} style={{ marginBottom: 14 }}>
                  ...
                  <div className="form-group">
                    <label className="form-label">Fatura Türü</label>
                    <select className="form-select" value={duzenleForm.fatura_turu} onChange={e => setDuzenleForm(f => ({ ...f, fatura_turu: e.target.value }))}>
                      <option value="">Belirtilmemiş</option>
                      <option value="MF">Müşteri Faturalı (MF)</option>
                      <option value="AF">Adına Fatura (AF)</option>
                    </select>
                ...
                <>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 6 }}>
                    ...
                    {c.fatura_turu && <Chip color="orange">{c.fatura_turu}</Chip>}
                  </div>
              ...
              {showMasraf && (
                <form onSubmit={submitMasraf} style={{ marginTop: 10 }}>
                  <div className="form-group">
                    ...
                    <input className="form-input" required value={masrafForm.aciklama} onChange={e => setMasrafForm({ ...masrafForm, aciklama: e.target.value })} placeholder="Aksesuar, kılıf, cam, ekran koruyucu..." />
                  </div>
